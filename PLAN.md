...
**一处由本问题直接引发的混淆（已核实更正）**：某子 agent 报告里把响应中 `clip_score=0.0` 解释为「数据本身没有向量（67400/68349 行 `clip_image_vector` 为 null）」。**该解释不成立**：
- `output/clip_vectors.faiss` 的 `ntotal = 68349`，**恰好等于 detection 总数** → 检索用的 FAISS 索引**覆盖 100%**（主 agent 实测）。
- 检索响应里的 `clip_score=0.0` 的真正含义是**该候选不在 CLIP top-200 内**（`api/routes/search.py` 只取 top-200 填 `clip_scores`，未命中者取默认 0.0），**不是"没有向量"**。
- 根因正是本节所述的两套特征空间：子 agent 混淆了 **768 维内联向量（builder 用，覆盖 1.39%）** 与 **512 维 FAISS 索引（检索用，覆盖 100%）**。
- **结论：§0.5 的特征空间分裂不仅削弱算法，还已经实际造成了一次分析误判**，进一步说明统一它的必要性。
