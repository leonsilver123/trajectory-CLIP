...
> **这个分裂已经实际造成了一次分析误判**：重构中曾把检索响应里 `clip_score=0.0` 解释成"该检测没有向量"。实测更正：`clip_vectors.faiss` 的 `ntotal=68349` **恰好等于检测总数，覆盖 100%**；`clip_score=0.0` 的真正含义是**该候选不在 CLIP top-200 内**（`search.py` 只取 top-200 回填，未命中取默认 0）。误判的根因正是混淆了这两套空间 —— 连参与重构的 agent 都会踩，可见其危害不只停留在算法精度上。
