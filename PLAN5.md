...
- `[x]` **F1** 跑全套评测并产出**本数据集口径**的指标表（P@10 / mAP / F1 / IDF1）

### 📊 F1 结果：AICity22 口径指标表（**与简历/文档的业务数据集数字不是同一套，不得混用**）

| 评测 | 脚本 | 指标 | 值 |
|---|---|---|---|
| 文本检索 | `eval_retrieval_hit_rate.py` | 命中率@20 | **0.4220**（92/218） |
| | | MRR | **0.1111** |
| | | 属性粗筛保留率 | 1.0000 |
| 跨镜观测链 | `eval_chain_idf1.py` | **IDF1（micro / macro）** | **0.5604 / 0.5142** |
| | | IDP / IDR | **0.9731 / 0.3935** |
| | | 摄像头完全一致 | **0.0000**（0/218） |
| | | 平均摄像头召回 / 精度 | **0.2891 / 0.9931** |
| 全局聚类 | `eval_global_idf1.py` | 官方 IDF1（motmetrics） | **0.4873** |
| | | 候选边总数 | **154**（926 个 tracklet 只聚成 868 条轨迹 ⇒ 仅 58 次合并） |
| 属性一致率 | `eval_attribute_consistency.py` | 跨镜一致（车型 / 颜色） | **28.4% / 37.2%** |
| | | 内一致（车型 / 颜色） | 74.4% / 81.8% |
| | | k=2 随机基线（车型 / 颜色） | 24.6% / 21.2% |
| | | PLAN2 验收目标 | 跨镜 > 60% ⇒ **未达标** |

**三条互相独立的证据指向同一个结论**（外观/属性信号在本数据集上太弱，而门控是按精度调的）：

1. **C4 直接测量**：同车跨镜对的平均 ReID 余弦中位数 0.780，而门控阈值 0.95 ⇒ **只有 0.9% 的真连接能过**
2. **`eval_chain_idf1`**：IDP **0.973** / IDR **0.394** —— 高精度、低召回，正是"门控为精度调的"的指纹。逐车看，多数车 GT 跨 8–24 个摄像头而**命中的只有 1 个**
3. **`eval_global_idf1`**：926 个 tracklet 只产生 **154 条候选边**、仅 58 次合并 —— 门控产不出边，聚类自然无从合并

**属性侧同样是弱信号**：车型跨镜一致率 28.4% 只比 k=2 随机基线 24.6% 高 **3.8 个点**，基本没超出随机；颜色 37.2% 对 21.2% 稍好。两者都远低于 PLAN2 定的 60% 目标。

**未做**：门控阈值仍是 0.95。C4 测过阈值降到 0.75 能放行 61.8% 的真连接 —— 但那会拿精度换召回、且改变线上行为，**应由项目负责人决定，不在本次范围内顺手调**。
