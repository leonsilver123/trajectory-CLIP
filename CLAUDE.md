...
- `webapp/` — the frontend (React 18 + TypeScript + Vite + antd; pages: Search / Backtrack / Trajectory / Dashboard, axios client in `src/api/`). Built to `webapp/dist/` and served by the backend on the same origin. Its `README.md` is still the unedited Vite template. **This is the only frontend** — the earlier Streamlit UI (`frontend/`) was removed on 2026-09-21.
