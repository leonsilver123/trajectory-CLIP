...
export default function TrajectoryPage() {
  ...
  // 深链接自动查询：从检索页 / 回溯页跳进来时 URL 已带 track_id 或 instance_id，
  // 用户意图是明确的，不该再让他点一次「查询轨迹」。
  // 用 ref 守卫，避免 run() 内部的 setParams 触发重渲染后再次进入（死循环）。
  const autoRan = useRef(false);
  useEffect(() => {
    if (autoRan.current) return;
    const hasTrack = (params.get("track_id") ?? "").trim();
    const hasInstance = (params.get("instance_id") ?? "").trim();
    if (!hasTrack && !hasInstance) return;
  ...
  const segColumns: ColumnsType<TrajectoryCameraSegment> = [
    ...
    },
    {
      ...
      render: (v: number | null | undefined) => (
        <Value value={v} digits={0} fallback="未知" />
      ),
    },
    {
      title: "方向",
    ...
    <Space direction="vertical" size={16} style={{ width: "100%" }}>
      ...
      {!loading && traj && (
        <>
          ...
          {/* 轨迹还原回放：把离散观测链按时间放起来；跨摄像头处不做插值（见组件注释） */}
          <TrajectoryPlayer trajectory={traj} />
