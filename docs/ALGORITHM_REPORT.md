...
> ⚠️ **本节数字来自两套不同的数据集，已经分开标注 —— 引用时不要合并。**
>
> - **业务数据集（苏州相城区，48 路）**：系统实际部署的环境。该数据集**不在本仓库内**，
>   其数字无法在此复现。
> - **本仓库数据集 AICity22**：`output/` 下的 68,349 条检测 / 230 个车辆身份 /
>   46 个摄像头。这些数字可核对、可用 `scripts/eval_*.py` 现跑现取。
>
> 两者规模与成像条件差别很大，**数字不可互换、不可比较**。AICity22 上的实测结果
> 通常**差得多**，且原因多在数据而非算法：裁剪图中位数仅 **118×98 像素**，
> 跨镜 ReID 的 d-prime 只有 **0.78**，且该数据集**完全没有车牌**
> （68,349 条检测中带 `plate_number` 的为 **0 条**）—— 这会让六维评分里的
> 车牌一致性维度**恒为无证据**、被整维剔除，实际生效的只有时间与拓扑两维。

### 7.1 检索性能

| 指标 | 数值 | 来源 | 说明 |
|------|------|------|------|
| P@10 | 0.973 | **业务数据集**（苏州相城区） | 模糊搜索前 10 命中率 |
| IDF1 | 0.987 | ⚠️ **来源未在本仓库中记录** | 跨镜跟踪 F1 分数 |
| unknown 率 | 0.2% | ⚠️ **来源未在本仓库中记录** | 批量推理中未识别目标比例（从 97.4% 优化） |

> 关于表中标注「来源未记录」的两项：**本文件作者未能找到它们的出处**。
> 它们既不对应本仓库 `scripts/eval_*.py` 的产出，也没有在别处被引用过。
> 之所以不猜，是因为猜错正是本节要防的事 —— 引用前请先确认，或改用
> `scripts/eval_*.py` 现跑。特别提醒：本仓库的 `eval_chain_idf1.py` 若用默认的
> `mode="auto"` 会解析成 `strong`（数据集自带真实 `vehicle_id`），IDF1 会是
> 平凡的 1.0，必须传 `mode="stitch"` 才有意义。

### 7.2 轨迹拼接（本仓库数据集 AICity22，可核对）
...
> 表中 230 个身份与 46 个摄像头可与本仓库数据逐一核对
> （`output/reid_train/summary.json` 的 `identities_total` = 230；
> `configs/cityflow_camera_metadata.yaml` 含 46 个摄像头）。
> 因此本小节属于 **AICity22 口径**，与 7.1 的 P@10 不是同一套数据。
